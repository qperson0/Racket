...
(define(keyhand model key)
   (cond [(key=? key "up") (list (-y (first model)) (second model))]
         ...
         [(key=? key "w") (list (first model) (-y (second model)))]
         [(key=? key "s")  (list (first model) (+y (second model)))]
         [(key=? key "d")  (list (first model) (+x (second model)))]
         [(key=? key "a")   (list (first model) (-x (second model)))]
         [else model]))
